...
class FreeformDataEntryCell: BaseDataEntryCell, UITextFieldDelegate { //add new class -> enum!
    ...
    override class var numberOfLevels: Int { //cell has 2 layers (topLayer + 1 for textLabel)
        return 2 //# of levels needs to be dynamic if there is > 2 labels in the cell
    }
...
//    private var characterLimit: Int? //limit # of characters
    ...
    private var freeformViewsConfigObject: [(String?, ProtectedFreeformTypes?, String?, Int?, (Int?, Int?)?)] = [] //'freeformView' = TF + lbl contained w/in a view; this tuple specifies all config for the view - indicates (1) # of TFs (via the array's count); (2) label for each TF; (3) type of data in TF? (if nil, default is String); (4) defaultValue?; (5) characterLimit?; (6) (if text is an Int) an upper/lower bound in format (Int?, Int?)?.
    private var labelBeforeField: Bool = true //if TRUE, labels come before TF; if FALSE, they come after
    
    //Dynamically adjust the cell - if there is only 1 TF (default configuration), keep default visuals as set now. Allow maximum of 2 TF per level.
